from goal_diffusion import GoalGaussianDiffusion, Trainer
from unet import UnetMW as Unet
from transformers import CLIPTextModel, CLIPTokenizer
import torch
import numpy as np
import os 
from torch.utils.data import Dataset, DataLoader, Subset
import torch.nn as nn
import torch.nn.functional as F
import json
from PIL import Image
import tqdm
from accelerate import Accelerator
from datasets import SequentialDataset, SequentialDatasetVal
import argparse
from einops import rearrange


def main(args):
    valid_n = 1
    sample_per_seq = 8
    target_size = (128, 128)

    if args.mode == 'inference':
        train_set = valid_set = [None] # dummy
    else:
        train_set = SequentialDatasetv2(
            sample_per_seq=sample_per_seq, 
            path="../datasets/metaworld", 
            target_size=target_size,
            randomcrop=True
        )
        valid_inds = [i for i in range(0, len(train_set), len(train_set)//valid_n)][:valid_n]
        valid_set = Subset(train_set, valid_inds)

    unet = Unet()

    pretrained_model = "openai/clip-vit-base-patch32"
    tokenizer = CLIPTokenizer.from_pretrained(pretrained_model)
    text_encoder = CLIPTextModel.from_pretrained(pretrained_model)
    text_encoder.requires_grad_(False)
    text_encoder.eval()

    diffusion = GoalGaussianDiffusion(
        channels=3*(sample_per_seq-1),
        model=unet,
        image_size=target_size,
        timesteps=100,
        sampling_timesteps=args.sample_steps,
        loss_type='l2',
        objective='pred_v',
        beta_schedule = 'cosine',
        min_snr_loss_weight = True,
    )

    trainer = Trainer(
        diffusion_model=diffusion,
        tokenizer=tokenizer, 
        text_encoder=text_encoder,
        train_set=train_set,
        valid_set=valid_set,
        train_lr=1e-4,
        train_num_steps =70000,
        save_and_sample_every =210,
        ema_update_every = 10,
        ema_decay = 0.999,
        train_batch_size =4,
        valid_batch_size =4,
        gradient_accumulate_every = 1,
        num_samples=valid_n, 
        results_folder ='/home/achint/AVDC_2/results/mw',
        fp16 =True,
        amp=True,
    )

    if args.checkpoint_num is not None:
        trainer.load(args.checkpoint_num)
    
    
    from PIL import Image
    from torchvision import transforms
    import imageio
    import torch
    from os.path import splitext
    text = args.text
    image = Image.open(args.inference_path)
    batch_size = 1
    transform = transforms.Compose([
            transforms.Resize((240, 320)),
            transforms.CenterCrop(target_size),
            transforms.ToTensor(),
        ])
    save_path = f'/home/achint/AVDC_2/examples/{args.text}_checkpoint_{args.checkpoint_num}'
    inference_steps = 5
    image = transform(image)
    if not args.refine:
        samples = 1
        for i in range(samples):
            output = trainer.sample(image.unsqueeze(0), [text], batch_size).cpu()
            # print(output.shape)
            output_1 = output[0].reshape(-1, 3, *target_size)
            output_1 = torch.cat([image.unsqueeze(0), output_1], dim=0)
            root, ext = splitext(save_path)
            output_gif = root + f'_out_{inference_steps}iter_{i}.gif'
            output_1 = (output_1.cpu().numpy().transpose(0, 2, 3, 1).clip(0, 1) * 255).astype('uint8')
            output_1 = [np.array(Image.fromarray(frame).resize((320, 240))) for frame in output_1]
            imageio.mimsave(output_gif, output_1, duration=200, loop=1000)
            print(f'Generated {output_gif}')
            for j in range(inference_steps):
                output = rearrange(output, 'b (f c) h w -> b c f h w', c = 3)
                output = trainer.sample(output, [text], batch_size).cpu()
                output_1 = output[0].reshape(-1, 3, *target_size)
                output_1 = torch.cat([image.unsqueeze(0), output_1], dim=0)
                root, ext = splitext(save_path)
                output_gif = root + f'_out_{inference_steps}iter_{i}{j}.gif'
                output_1 = (output_1.cpu().numpy().transpose(0, 2, 3, 1).clip(0, 1) * 255).astype('uint8')
                output_1 = [np.array(Image.fromarray(frame).resize((320, 240))) for frame in output_1]
                imageio.mimsave(output_gif, output_1, duration=200, loop=1000)
                print(f'Generated {output_gif}')
                
                # output = trainer.sample(image.unsqueeze(0), [text], batch_size, guidance_weight).cpu()
                # output = output[0].reshape(-1, 3, *target_size)
                # output = torch.cat([image.unsqueeze(0), output], dim=0)
                # root, ext = splitext(args.inference_path)
                # output_gif = root + '_out.gif'
                # output = (output.cpu().numpy().transpose(0, 2, 3, 1).clip(0, 1) * 255).astype('uint8')
                # imageio.mimsave(output_gif, output, duration=200, loop=1000)
                # print(f'Generated {output_gif}')
    
    else:
        inference_steps = 5
        gif_path = '/home/achint/AVDC/examples/take clothes out of laundry machine_checkpoint_3422_out_0iter_0.gif'  # Update the path accordingly

        # Read the GIF from file
        frames = imageio.mimread(gif_path)
        
        frames_tensor = torch.stack([
        transform(Image.fromarray(frame))  # Create a PIL image from each numpy array and apply the transform
        for frame in frames[1:]  # Skip the first frame
        ])
        # Normalize pixel values (Optional, depending on your model)
        frames_tensor = frames_tensor.float() / 255.0
        # frames_tensor = frames_tensor[1:,:,:,:]
        print(frames_tensor.shape)
        output = frames_tensor.unsqueeze(0)
        # image = gif_tensor[:, 1, :, :, :]
        for i in range(inference_steps):
            # print(output.shape)
            if i == 0:
                output = rearrange(output, 'b f c h w -> b c f h w', c = 3)
            else:
                output = rearrange(output, 'b (f c) h w -> b c f h w', c = 3)
            # print(output.shape)
            output = trainer.sample(output, [text], batch_size).cpu()
            output_1 = output[0].reshape(-1, 3, *target_size)
            output_1 = torch.cat([image.unsqueeze(0), output_1], dim=0)
            root, ext = splitext(save_path)
            output_gif = root + f'_out_{i}.gif'
            output_1 = (output_1.cpu().numpy().transpose(0, 2, 3, 1).clip(0, 1) * 255).astype('uint8')
            output_1 = [np.array(Image.fromarray(frame).resize((320, 240))) for frame in output_1]
            imageio.mimsave(output_gif, output_1, duration=200, loop=1000)
            print(f'Generated {output_gif}')
        

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-m', '--mode', type=str, default='inference', choices=['train', 'inference']) # set to 'inference' to generate samples
    parser.add_argument('-c', '--checkpoint_num', type=int, default=None) # set to checkpoint number to resume training or generate samples
    parser.add_argument('-p', '--inference_path', type=str, default=None) # set to path to generate samples
    parser.add_argument('-t', '--text', type=str, default=None) # set to text to generate samples
    parser.add_argument('-n', '--sample_steps', type=int, default=100) # set to number of steps to sample
    parser.add_argument('-g', '--guidance_weight', type=int, default=0) # set to positive to use guidance
    parser.add_argument('-r', '--refine', type=bool, default=False) # set to positive to use guidance
    args = parser.parse_args()
    if args.mode == 'inference':
        assert args.checkpoint_num is not None
        # assert args.inference_path is not None
        assert args.text is not None
        assert args.sample_steps <= 100
    main(args)